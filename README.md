...
- **Cinematic Experience**: Designed with a dark, nostalgic palette, editorial typography, and tactile interactions to evoke the feeling of a physical luxury ticket stub.
...
- **Premium Animations**: Smooth, 60fps layout transitions, 3D flip effects, and subtle haptics powered by `react-native-reanimated` and `expo-haptics`.
...
- **Permissions**: A unified, contextual system that handles pre-request explanations and graceful fallback states.
...
- **Export Engine**: Uses `react-native-view-shot` to render components into high-fidelity images for sharing.
...
│   ├── editor.tsx        # High-density artifact creation interface
│   └── ticket/
│       └── [id].tsx      # Immersive cinematic detail screen
├── components/           # Reusable UI components
│   ├── TicketCanvas.tsx  # The core high-fidelity ticket UI
│   ├── TicketCard.tsx    # Animated wrapper for the Archive grid
│   ├── CategoryCreator.tsx # Modal for creating new categories
│   └── Toast.tsx         # Global feedback system
├── constants/            # Design system tokens
│   └── theme.ts          # Palette, Typography, Spacing, Shadows
├── hooks/                # Custom React hooks (useTickets, useDevice)
├── utils/                # Pure utility functions
│   ├── files.ts          # Permanent image file migration
